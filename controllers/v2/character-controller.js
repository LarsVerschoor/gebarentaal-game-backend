    ...
    getAll: async (req, res) => {
        try {
            ...
            const charactersWithLinks = characters.map(character => ({
                ...character.toJSON(),
                image_path: `${process.env.SERVER_URL}/uploads/${character.image_path}`,
                self: { href: `${process.env.SERVER_URL}/api/v2/characters/${character.id}` },
                collection: { href: `${process.env.SERVER_URL}/api/v2/characters` },
            }));
